...
def get_openml_classification(args,did, max_samples, multiclass=True, shuffled=True):
...
def load_openml_list(args,dids, filter_for_nan=False
                     , num_feats=100
                     , min_samples = 100
                     , max_samples=400
                     , multiclass=True
    ...
    for ds in datalist.index:
        ...
        if entry['NumberOfClasses'] == 0.0:
            ...
            X, y, categorical_feats, attribute_names = get_openml_classification(args,int(entry.did), max_samples
